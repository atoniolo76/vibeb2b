...
def process_image(image_data):
    """Process a single PNG frame and return metrics"""
    global lstm_features, metrics_history, face_mesh

    try:
        # Convert PNG data to OpenCV format
        if isinstance(image_data, str):
            # Base64 encoded PNG from WebSocket receiver
            ...
            # Direct image array (for testing)
            frame = image_data
...
def fetch_and_process_png_frames(image_queue):
    """Fetch PNG frames directly from PNG_realtime_receiver at 2fps"""
    # Import the PNG receiver's frame manager
    import sys
    import os
    sys.path.append(os.path.dirname(os.path.abspath(__file__)))
    
    # Wait for PNG receiver to be available
    frame_manager = None
    max_retries = 10
    retry_count = 0
    
    while frame_manager is None and retry_count < max_retries:
        try:
            from PNG_realtime_receiver import frame_manager
            print("✅ Connected to PNG receiver frame manager")
            break
        except ImportError:
            retry_count += 1
            print(f"⏳ Waiting for PNG receiver... ({retry_count}/{max_retries})")
            time.sleep(2)
    
    if frame_manager is None:
        print("❌ Cannot connect to PNG receiver - make sure it's running")
        return
    
    while True:
        try:
            # Get latest frame directly from frame manager (much faster!)
            ...
            if frame is not None:
                ...
                if not image_queue.full():
                    image_queue.put(frame_base64)
                    stats = frame_manager.get_stats()
                    print(f"📸 Processing frame | FPS: {stats['receive_fps']} | Skip ratio: {stats['skip_ratio']}%")
                else:
                    print("⚠️ Processing queue full - skipping frame")
            else:
                print("📭 No frame available yet...")

            time.sleep(0.5)  # 2fps processing rate
            
        except Exception as e:
            print(f"Error fetching PNG frame: {e}")
            time.sleep(1)
...
def process_single_image():
    """API endpoint to process a single PNG frame (for testing)"""
    try:
        ...
        elif 'png_data' in request.json:
            # Handle PNG base64 data (primary method)
            png_data = request.json['png_data']
            png_bytes = base64.b64decode(png_data)
            nparr = np.frombuffer(png_bytes, np.uint8)
            image_array = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
            if image_array is None:
                return jsonify({"error": "Failed to decode PNG data"}), 400
        elif 'image_data' in request.json:
            # Fallback for old format
            image_data = request.json['image_data']
            image_array = base64.b64decode(image_data)
        else:
            return jsonify({"error": "No PNG data provided. Use 'png_data' field with base64 PNG."}), 400
...
def start_auto_processing():
    """Automatically start processing when API server starts"""
    global processing_active, processing_threads
    
    if processing_active:
        return
    
    print("🚀 Auto-starting PNG frame processing...")
    
    # Create queue for PNG frames (smaller queue for latest-only strategy)
    image_queue = queue.Queue(maxsize=3)

    # Start background threads
    fetch_thread = threading.Thread(target=fetch_and_process_png_frames, args=(image_queue,), daemon=True)
    process_thread = threading.Thread(target=process_image_queue, args=(image_queue,), daemon=True)

    fetch_thread.start()
    process_thread.start()
    
    processing_threads = [fetch_thread, process_thread]
...
def start_processing():
    """Manual start processing (for compatibility)"""
    if not processing_active:
        start_auto_processing()
        return jsonify({"message": "Started processing PNG frames", "strategy": "latest_only"})
    else:
        return jsonify({"message": "Processing already active", "strategy": "latest_only"})
...
if __name__ == '__main__':
    ...
    if load_models():
        ...
        print("📡 Auto-connecting to PNG receiver...")
        
        # Start auto-processing in background
        auto_start_thread = threading.Thread(target=start_auto_processing, daemon=True)
